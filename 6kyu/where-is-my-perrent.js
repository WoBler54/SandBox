...
function findChildren(dancingBrigade) {
    //	console.log(dancingBrigade.sort());
    
       let arrWithDancing = dancingBrigade.toLowerCase().split('').sort();
       let result = '';
       result += arrWithDancing[0].toUpperCase();
       for(let i = 1; i < arrWithDancing.length; i++){
             if(arrWithDancing[i] != arrWithDancing[i+1] && arrWithDancing[i+1] != undefined){
                result += arrWithDancing[i];
                result += arrWithDancing[i + 1].toUpperCase();
                i++;
             }else {
                result += arrWithDancing[i];
             }
